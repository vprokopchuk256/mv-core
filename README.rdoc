...
= Usage
  ...
  validate_column :str_column, :integer, :exclusion => {:in => [1,2,3]}
  ...
  validate_column :str_column, :integer, :exclusion => false

= Drivers

  Currently there are drivers for MySQL, PostgreSQL and SQLite RDBMS
  
  list of supported validators my vary for each db (it depends on RDBMS features actually)

  so - see detailed info here: 

  <a href="https://github.com/vprokopchuk256/mv-postgresql">PosgreSQL</a>
